...
def get_aggregation_by_consigner_code(consigner_code = "HZL", field = "iron_quantity_kg", op = "avg")
  ...
  logger.info("ES API FETCHING STARTED AT #{(Time.now + (5.5 * 60 * 60)).to_s.sub!('+0000', '')}")
  begin
    ...
    logger.info("ES API ERROR BECAUSE OF #{e.message} AT #{(Time.now + (5.5 * 60 * 60)).to_s.sub!('+0000', '')}")
